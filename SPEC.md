...
V12: pinned-lab5-qms — rebuild publishes Lab5.QMS from `customization/Lab5.QMS.pin` (GitHub release tag + sha256) via `QMS_SRC` `lab5-qms deploy`; `gmake qms-update` latest release → pin tag+sha256 + `.cache/` zip; this repo ! compile DLL ! vendor zip ! `acu check`
...
T26|.|add NewSymbol '<NEW>' on QORD+QNCR; `acu apply config/qms/05-numbering-sequences.yaml` insert + re-apply no 422|V8,B4
